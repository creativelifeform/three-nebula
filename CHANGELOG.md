...
- Migrated the build from Babel/webpack to Vite, and the tests from Mocha to Vitest

### Fixed

- `fromJSON` / `fromJSONAsync` now read the emitter `damping` property, which was previously ignored and silently reverted to the default on load
